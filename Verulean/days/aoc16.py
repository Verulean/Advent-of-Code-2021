...
class BITSPacket:
    ...
    def __str__(self):
        subs = (str(sub) for sub in self.subpackets)
        if self.type == 0:
            return f"sum([{', '.join(subs)}])"
        elif self.type == 1:
            return f"prod([{', '.join(subs)}])"
        elif self.type == 2:
            return f"min([{', '.join(subs)}])"
        elif self.type == 3:
            return f"max([{', '.join(subs)}])"
        elif self.type == 4:
            return str(self.value)
        elif self.type == 5:
            return f"({str(self.subpackets[0])} > {str(self.subpackets[1])})"
        elif self.type == 6:
            return f"({str(self.subpackets[0])} < {str(self.subpackets[1])})"
        elif self.type == 7:
            return f"({str(self.subpackets[0])} == {str(self.subpackets[1])})"
